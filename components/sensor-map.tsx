...
export function SensorMap({ sensors, onSensorSelect }: SensorMapProps) {
  ...
  const handleSensorClick = (sensor: Sensor) => {
    ...
    setClickedLocation(null); // Clear clicked location when selecting a sensor
    onSensorSelect?.(sensor);
  };

  const handleMapClick = (location: ClickedLocation) => {
    setClickedLocation(location);
    setSelectedMarker(null); // Clear sensor selection when clicking map
  };
    ...
    <Card className="col-span-full overflow-hidden border-white/[0.06] bg-[#080c1a]/60 backdrop-blur-xl shadow-2xl shadow-black/20">
      <CardHeader className="flex flex-row items-start justify-between gap-4 flex-wrap pb-4 border-b border-white/[0.04]">
        <div>
          ...
          <CardDescription className="text-xs text-white/30">India — Real-time Infrastructure Monitoring · Click anywhere on the map to pin a location</CardDescription>
        </div>
      ...
      <CardContent className="p-0">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px]">
          ...
          <div className="border-l border-white/[0.04] bg-[#070b18]/80 p-4 space-y-4 max-h-[560px] overflow-y-auto">
            ...
            {clickedLocation && (
              <div className="rounded-xl bg-indigo-500/[0.06] border border-indigo-500/20 p-3.5" style={{ boxShadow: '0 0 20px rgba(99,102,241,0.08)' }}>
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-2.5 h-2.5 rounded-full bg-indigo-400 flex-shrink-0" style={{ boxShadow: '0 0 8px rgba(99,102,241,0.6)' }} />
                  <h3 className="font-bold text-xs text-white/90 uppercase tracking-widest">Pinned Location</h3>
                </div>
                <div className="space-y-2.5 text-sm">
                  <div className="border-b border-white/[0.04] pb-2">
                    <p className="text-white/25 text-[10px] mb-0.5 uppercase tracking-wider font-bold">Latitude</p>
                    <p className="font-semibold text-xs text-indigo-300 font-mono">{clickedLocation.lat.toFixed(5)}</p>
                  </div>
                  <div className="border-b border-white/[0.04] pb-2">
                    <p className="text-white/25 text-[10px] mb-0.5 uppercase tracking-wider font-bold">Longitude</p>
                    <p className="font-semibold text-xs text-indigo-300 font-mono">{clickedLocation.lng.toFixed(5)}</p>
                  </div>
                </div>
                <button
                  onClick={() => setClickedLocation(null)}
                  className="mt-3 w-full px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-white/40 hover:text-white/70 bg-white/[0.04] hover:bg-white/[0.08] rounded-lg border border-white/[0.06] transition-all"
                >
                  Clear Pin
                </button>
              </div>
            ...
            {selectedSensor ? (
              ...
              <div className="rounded-xl bg-white/[0.02] border border-dashed border-white/[0.06] p-5 text-center">
                ...
                <p className="text-[10px] text-white/20">Click a marker or anywhere on the map</p>
              </div>
            )}
          </div>
        </div>
